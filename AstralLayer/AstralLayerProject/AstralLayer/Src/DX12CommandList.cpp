    ...
    AstralLayer::ATLIRenderTargetView* rtv)
{
    ...
    handle.ptr +=
        static_cast<unsigned long>(m_BackBuffer) *
        static_cast<unsigned long>(device->GetDescriptorHandleIncrementSize(D3D12_DESCRIPTOR_HEAP_TYPE_RTV));
    ...
    AstralLayer::ATLIFence* pFence)
{
    ...
    static_cast<AstralRHI::RHIPipeLine*>(pPipeLine)->
...
void AstralLayerDirectX12::DX12CommandList::SetGraphicsRootSignature(
    AstralLayer::ATLIRootSignature* pRootSignature)
{
    //ルートシグネチャー取得
    ID3D12RootSignature* r = nullptr;
    reinterpret_cast<AstralRHI::RHIRootSignature*>(pRootSignature)->
        GetHandle(reinterpret_cast<void**>(&r), ROOTSIGNATURE_ROOTSIGNATURE);

    //セット
    m_pCommandList->SetGraphicsRootSignature(r);
}
    ...
    unsigned int* ResourceIDs)
{
    ...
    for (unsigned int i = 0; i < NumResourceIDs; i++)
    {
        ...
        handle.ptr +=
            ...
            static_cast<unsigned long>(device->GetDescriptorHandleIncrementSize(D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV));
