...
AstralLayer::ATLIRootSignature* AstralLayerDirectX12::DX12Device::CreateRootSignature(
    ATL_ROOT_SIGNATURE_DESC& Desc)
{
    //ルートシグネチャー作成
    DX12RootSignature* pOut = new DX12RootSignature();
    if (pOut->Create(m_pDevice, Desc) == false)
    {
        ATLAssertMessage(false, "ルートシグネチャーの作成に失敗しました");
        delete pOut;
        pOut = nullptr;
    }
    return pOut;
