	...
	ATL_GRAPHICS_PIPELINE_STATE_DESC& Desc)
{
	...
	if (pOut->Create(m_pDevice, pRootSignature, Desc) == false)
	{
...
AstralLayer::ATLIRootSignature* AstralLayerDirectX11::DX11Device::CreateRootSignature(
	ATL_ROOT_SIGNATURE_DESC& Desc)
{
	//ルートシグネチャー作成
	DX11RootSignature* pOut = new DX11RootSignature();
	if (pOut->Create(m_pDevice, Desc) == false)
	{
		ATLAssertMessage(false, "RootSignatureの作成に失敗しました");
		delete pOut;
		pOut = nullptr;
	}
	return pOut;
