...
namespace AstralLayerDirectX12
{
	...
	static const int ROOTSIGNATURE_ROOTSIGNATURE = 0;	//!< AstralDirectX12::DX12RootSignature::GetHandle()で利用する定数
	static const int ROOTSIGNATURE_NUMPARAMETOR = 1;	//!< AstralDirectX12::DX12RootSignature::GetHandle()で利用する定数

	/****************************************************************//**
	 * DX12ルートシグネチャー
	 *******************************************************************/
	class DX12RootSignature : public AstralRHI::RHIRootSignature
	{
	private:
		ID3D12RootSignature* m_pRootSignature = nullptr;	//!< ルートシグネチャー
		unsigned int m_NumParametor = 0;					//!< パラメーター数
